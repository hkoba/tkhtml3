...
* **Stage A — single-line flex** — **DONE (2026-07-12)**, landed as
  three commits (property plumbing cfa0498, layout engine a48087b,
  tests): `display: flex | inline-flex`, `flex-direction` (row/column
  ± reverse), `justify-content` (6 values), `align-items`/`align-self`
  (stretch/center/start/end; baseline renders as flex-start until
  stage B), `flex-grow/shrink/basis` + `flex` shorthand, `gap`
  (px/em/rem; % falls back), `order`, auto margins in both axes.
  Implementation followed the plan exactly: new `htmlflexlayout.c`
  (~900 lines incl. comments) using blockMinMaxWidth() measurement,
  the §9.7 loop in integer arithmetic, and the table-cell drawing
